        ...
        while self.running:
            ...
            if mode == Mode.FACES:
                ...
                if recognized_faces:
                    ...
                    print(f"[COLLECTION] Checking if should start collection for {person_name}")
                ...
                for person_name in list(self.active_collections.keys()):
                    ...
                    print(f"[EMBEDDING] Extracted embedding shape: {emb.shape}, took {emb_time:.1f}ms for {person_name} from {job.camera_id}")

                    # Save to DB + update counters
                    print(f"[DB] Attempting to save embedding for {person_name} from {job.camera_id}")
