...
class Lateral_Raise:
    def __init__(self):
        ...
        self.REP_COUNT_UP_THRESHOLD, self.REP_COUNT_DOWN_THRESHOLD = 60, 35
        self.FORM_UP_THRESHOLD, self.FORM_DOWN_THRESHOLD = 75, 30
        self.SWAY_THRESHOLD_X, self.KNEE_ANGLE_THRESHOLD = 25, 165
        self.SHOULDER_SHRUG_THRESHOLD, self.ARM_STRAIGHT_THRESHOLD = 0.25, 150
        self.OVER_SWING_THRESHOLD, self.WRIST_Y_ALIGNMENT_TOLERANCE = 100, 0.1

    def analyze_batch(self, df, *,
                      exercise_name=None,
                      analyzer_used="Lateral_Raise",
        ...
        for local_idx, (_, row) in enumerate(df.iterrows()):
            ...
            for side in ['left', 'right']:
                ...
                if row[f'{side}_elbow_angle'] < self.ARM_STRAIGHT_THRESHOLD:
                    current_rep_feedback[side].append(f"Bending {side} arm too much. Keep it straighter.")
                ...
                if raise_angle < self.REP_COUNT_DOWN_THRESHOLD and self.stages[side] == 'up':
                    if raise_angle_at_peak[side] < self.FORM_UP_THRESHOLD:
                        current_rep_feedback[side].append("Incomplete top range of motion. Lift to shoulder height.")
...
# --- (The feedback summary comment block remains the same) ---
...
# "Bending {side} arm too much. Keep it straighter."
#   - Trigger: The elbow angle becomes less than ARM_STRAIGHT_THRESHOLD.
#   - Why it's bad: Turns the isolation exercise into a compound movement, engaging biceps and reducing focus on the medial deltoids.
#
# "{Side} shoulder shrugging. Focus on your deltoids."
...
#
# "Incomplete top range of motion. Lift to shoulder height."
#   - Trigger: A rep is completed, but the peak raise angle achieved was less than the strict FORM_UP_THRESHOLD.
#   - Why it's bad: Fails to fully contract the medial deltoid, making the exercise less effective.
#
