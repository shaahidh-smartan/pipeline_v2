# modules/db_incline_chest_press.py
...
class DB_Incline_Chest_Press:
    def __init__(self):
        ...
        self.REP_COUNT_UP_THRESHOLD, self.REP_COUNT_DOWN_THRESHOLD = 150, 105
        self.FORM_UP_THRESHOLD, self.FORM_DOWN_THRESHOLD = 160, 95
        self.SHOULDER_LIFT_THRESHOLD = 25
        self.ELBOW_LOCKOUT_THRESHOLD = 175
        self.SHOULDER_FLARE_THRESHOLD = 80
        self.WRIST_Y_ALIGNMENT_TOLERANCE = 0.1
        self.INCLINE_MIN_ANGLE, self.INCLINE_MAX_ANGLE = 115, 145

    def analyze_batch(self, df, *,
                      exercise_name=None,
                      analyzer_used="DB_Incline_Chest_Press",
        ...
        baseline_shoulder_y = (df.iloc[0]['left_shoulder_y'] + df.iloc[0]['right_shoulder_y']) / 2
        ...
        for local_idx, (_, row) in enumerate(df.iterrows()):
            ...
            current_shoulder_y = (row['left_shoulder_y'] + row['right_shoulder_y']) / 2
            if abs(current_shoulder_y - baseline_shoulder_y) > self.SHOULDER_LIFT_THRESHOLD:
                msg = "Arching back too much. Keep your shoulders on the bench."
            ...
            if torso_height > 0:
                ...
                if y_diff_norm > self.WRIST_Y_ALIGNMENT_TOLERANCE:
                    msg = "Uneven press. Push both dumbbells at the same time."
                    current_rep_feedback['left'].append(msg)
                    current_rep_feedback['right'].append(msg)
            
            avg_hip_angle = (row['left_hip_angle'] + row['right_hip_angle']) / 2
            if not self.INCLINE_MIN_ANGLE < avg_hip_angle < self.INCLINE_MAX_ANGLE:
                msg = "Incorrect bench angle. Adjust your incline."
                current_rep_feedback['left'].append(msg)
                current_rep_feedback['right'].append(msg)

            for side in ['left', 'right']:
                if row[f'{side}_shoulder_angle'] > self.SHOULDER_FLARE_THRESHOLD:
                    current_rep_feedback[side].append(f"Flaring {side} elbow. Tuck it in slightly to protect your shoulder.")
                
                elbow_angle = row[f'{side}_elbow_angle']
                if elbow_angle > self.ELBOW_LOCKOUT_THRESHOLD:
                    current_rep_feedback[side].append(f"Hyperextending {side} elbow. Don't lock your arm at the top.")

                if elbow_angle < self.REP_COUNT_DOWN_THRESHOLD:
                    if self.stages[side] == 'up':
                        if elbow_angle_at_peak[side] < self.FORM_UP_THRESHOLD:
                            current_rep_feedback[side].append("Incomplete press. Extend your arms fully.")
                        if elbow_angle_at_bottom[side] > self.FORM_DOWN_THRESHOLD:
                             current_rep_feedback[side].append("Not lowering deep enough. Bring elbows to 90 degrees.")
...
#
# ERROR FEEDBACK (BODY):
# "Incorrect bench angle. Adjust your incline."
#   - Trigger: The average hip angle (shoulder-hip-knee) is outside the valid incline range (e.g., 115-145 degrees).
#   - Why it's bad: The user is either sitting too upright (turning it into a shoulder press) or is not inclined enough (turning it into a flat press), targeting the wrong part of the chest.
#
# "Arching back too much. Keep your shoulders on the bench."
#   - Trigger: The vertical midpoint of the shoulders moves up more than SHOULDER_LIFT_THRESHOLD from its starting position.
#   - Why it's bad: Reduces chest isolation, transfers stress to the lower back, and can risk injury.
#
# ERROR FEEDBACK (ARMS & SHOULDERS):
# "Uneven press. Push both dumbbells at the same time."
...
#
# "Flaring {side} elbow. Tuck it in slightly to protect your shoulder."
#   - Trigger: The shoulder angle (hip-shoulder-elbow) is greater than SHOULDER_FLARE_THRESHOLD.
#   - Why it's bad: Places the rotator cuff in a vulnerable position, increasing the risk of shoulder impingement and injury.
#
# "Hyperextending {side} elbow. Don't lock your arm at the top."
#   - Trigger: The elbow angle exceeds the ELBOW_LOCKOUT_THRESHOLD.
#   - Why it's bad: This is a major safety risk that places extreme stress on the elbow joint.
#
# "Incomplete press. Extend your arms fully."
#   - Trigger: A rep is completed, but the peak elbow angle achieved was less than the strict FORM_UP_THRESHOLD.
#   - Why it's bad: The user is not completing the concentric part of the lift, missing out on upper-chest and tricep activation.
#
# "Not lowering deep enough. Bring elbows to 90 degrees."
#   - Trigger: A rep is completed, but the lowest elbow angle achieved was greater than the strict FORM_DOWN_THRESHOLD.
#   - Why it's bad: This is a "partial rep" that doesn't fully stretch the pectoral muscles, reducing the overall effectiveness.
#