...
class ImagePartBasedEngine(Engine):
    ...
    def _evaluate(
        ...
        if save_features:
            ...
            torch.save(g_pids, osp.join(features_dir, 'gallery_pids_' + dataset_name + '.pt'))
            torch.save(q_pids, osp.join(features_dir, 'query_pids_' + dataset_name + '.pt'))
            torch.save(g_camids, osp.join(features_dir, 'gallery_camids_' + dataset_name + '.pt'))
            torch.save(q_camids, osp.join(features_dir, 'query_camids_' + dataset_name + '.pt'))
